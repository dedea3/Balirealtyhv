    ...
    <div class="grid grid-cols-4 gap-4">
        <div>
            ...
            <input type="number" name="bedroom_configs[{{ $index }}][bedroom_count]"
                   value="{{ old('bedroom_configs.' . $index . '.bedroom_count', $config['bedroom_count'] ?? ($existing ? $config->bedroom_count : '')) }}"
        ...
        <div>
            ...
            <input type="number" name="bedroom_configs[{{ $index }}][price_per_night]"
                   value="{{ old('bedroom_configs.' . $index . '.price_per_night', $config['price_per_night'] ?? ($existing ? $config->price_per_night : '')) }}"
        ...
        <div>
            ...
            <input type="number" name="bedroom_configs[{{ $index }}][min_nights]"
                   value="{{ old('bedroom_configs.' . $index . '.min_nights', $config['min_nights'] ?? ($existing ? $config->min_nights : 1)) }}"
                   class="input-field text-sm py-2" required>
        </div>
        <div class="flex items-end">
            <label class="flex items-center">
                <input type="checkbox" name="bedroom_configs[{{ $index }}][is_active]" value="1"
                       {{ old('bedroom_configs.' . $index . '.is_active', isset($config['is_active']) ? $config['is_active'] : ($existing ? $config->is_active : true)) ? 'checked' : '' }}
